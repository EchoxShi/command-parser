...
function executeCommand(command: string, shell: ShellType): void {
...
function getShell(optionShell?: string): ShellType {
  ...
  if (savedShell && ['powershell', 'cmd', 'bash'].includes(savedShell)) {
    return savedShell as ShellType;
  }
...
program
  ...
  .action(async (query: string | undefined, options: { shell?: string; explain: boolean; yes?: boolean }) => {
...
program
  ...
  .action((options: { setKey?: string; setUrl?: string; setModel?: string; setShell?: string; show?: boolean; clear?: boolean }) => {
    if (options.setKey) {
    ...
    if (options.setShell) {
      ...
      configStore.set('defaultShell', options.setShell as ShellType);
      configStore.set('shellDetected', true);
