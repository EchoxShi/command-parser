 ...
 */
export function getDetailedPlatform() {
    const platform = process.platform;
    if (platform === 'darwin')
        return 'macOS (BSD-based, use macOS-specific commands like ifconfig, pbcopy, open)';
    if (platform === 'linux')
        return 'Linux (use GNU coreutils, ip/hostname -I for network)';
    if (platform === 'win32') {
        if (process.env.MSYSTEM)
            return 'Windows Git Bash (MINGW, limited Linux command support)';
        return 'Windows';
    }
    return platform;
}
